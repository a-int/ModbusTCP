...
static void mb_process_read_fn(char* mb_repl_buf, char* mb_req_buf,  uint16_t req_buf_len){
	...
	if(mb_process_start_address(fn, start_address, quantity) != MB_EXCEPTION_OK){
		...
		start_address = mb_start_offset(fn, start_address);
		for(uint16_t i = 0; i != quantity; ++i){
//			 mb_repl_buf[MB_PDU_VAL_START + i] = source[start_address +i];
		}
	}
}
...
static int mb_process_start_address(uint16_t fn, uint16_t start_address, uint16_t quantity){
	...
	switch(fn){
		case MB_FN_READ_COILS:
			if((start_address + quantity) > MB_COILS_Q)			exception_code = 2;
			break;
		case MB_FN_READ_DISCRETE:
			if((start_address + quantity) > MB_DISCRETE_Q)	exception_code = 2;
			break;
		case MB_FN_READ_HOLDING:
			if((start_address + quantity*2) > MB_HOLDING_Q)		exception_code = 2;
			break;
		case MB_FN_READ_INPUT:
			if((start_address + quantity*2) > MB_INPUT_Q)			exception_code = 2;
			break;
	}
